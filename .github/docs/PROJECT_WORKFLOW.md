...
| **Feature** | Single repo deliverable | Days–weeks | `feature` | (same as parent epic branch) | "Add AuthToken contract types" |
| **Task** | Single PR unit of work | Hours–days | `task` | `subfeature/task/<desc>` → PR to `feature/*` | "Define AuthTokenPayload Zod schema" |
| **Bug** | Bug fix within a feature | Hours–days | `bug` | `subfeature/bugfix/<desc>` → PR to `feature/*` | "Fix schema drift in auth types" |
| **Refactor** | Code restructuring | Hours–days | `refactor` | `subfeature/refactor/<desc>` → PR to `feature/*` | "Split contract index exports" |
| **Test** | Test additions/fixes | Hours–days | `test` | `subfeature/test/<desc>` → PR to `feature/*` | "Add contract validation tests" |
| **Docs** | Documentation | Hours–days | `docs` | `subfeature/docs/<desc>` → PR to `feature/*` | "Document auth contract types" |
| **Chore** | Maintenance/tooling | Hours–days | `chore` | `subfeature/chore/<desc>` → PR to `feature/*` | "Update vitest config" |
...
2. **Create Feature issues** as sub-issues of the Epic
   ...
   - Sub-issues can be in **different repos** — a Backend Epic can have Interface sub-issues
   - Label: `feature`
